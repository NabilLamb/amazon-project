...
export function renderPaymentSummary() {
  ...
  cart.forEach((cartItem) => {
    ...
    shippingPriceCents += deliveryOption.princeCents * cartItem.quantity;
  });

  const totalPriceCentsBeforeTax = productPriceCents + shippingPriceCents;

  const taxCents = totalPriceCentsBeforeTax * 0.1;

  const totalPriceCents = totalPriceCentsBeforeTax + taxCents;
